...
const sections = [
  ...
  ["Contacto", "El responsable del servicio es Mauricio Correa, con operación en Colombia. Las consultas sobre estos términos pueden enviarse a privacy@zoepic.online."],
];
export default function TermsPage() { return <PublicShell consent={false}><main className="reading px-5 py-16 sm:px-6 md:py-24"><h1 className="font-display text-5xl font-medium">Términos de uso</h1><p className="mt-3 text-sm text-muted-foreground">Última actualización: 11 de septiembre de 2026</p><div className="mt-12 space-y-10">{sections.map(([title, body]) => <section key={title}><h2 className="text-xl font-semibold">{title}</h2><p className="mt-3 leading-7 text-foreground/80">{body}</p></section>)}</div></main></PublicShell>; }
