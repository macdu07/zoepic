...
export interface GuideExperimentVariant { label: string; width: number; height: number; quality: number; }
export interface GuideExperiment {
  ...
  methodology: string;
  limitations: string;
  variants: GuideExperimentVariant[];
}
...
export const guides: Guide[] = [
  {
    ...
    experiment: {
      sourcePath: "/guides/zoepic-muestra-producto.png",
      sourceBytes: 21369,
      browserLabel: "Chrome de escritorio",
      title: "Una conversión que puedes repetir",
      description: "La misma muestra se procesa localmente con la calidad indicada y cada salida queda disponible para descargar.",
      attribution: "Muestra propia de ZoePic; no contiene material de terceros.",
      methodology: "PNG de 1200 × 800 px, convertido en este navegador a WebP con calidad 82.",
      limitations: "El tamaño final puede cambiar según el navegador y el codificador disponible.",
      variants: [{ label: "WebP para publicar", width: 1200, height: 800, quality: 82 }],
    },
    sections: [
      ...
      { heading: "Un flujo de trabajo reproducible", paragraphs: ["Conserva los originales en una carpeta separada. Trabaja sobre copias, agrupa imágenes que tendrán el mismo uso y aplica una configuración común. En ZoePic puedes mantener las dimensiones o elegir una salida concreta antes de convertir el lote.", "Para esta guía usamos una muestra propia de 1200 × 800 px (21.369 bytes) y Chrome de escritorio. La convertimos a WebP con calidad 82 desde el navegador, sin subir el PNG. El peso que ves en la tarjeta se calcula al abrir la guía y puede variar con el navegador y su codificador; por eso la cifra es reproducible como método, no como promesa universal."], bullets: ["Selecciona JPG, JPEG o PNG del mismo contexto.", "Empieza con calidad 82–90 para fotografía y revisa detalles finos.", "Comprueba dimensiones, peso y transparencia en cada resultado.", "Descarga solo las versiones que aportan un ahorro útil."] },
  ...
  },
  {
    ...
    experiment: {
      ...
      sourceBytes: 21369,
      browserLabel: "Chrome de escritorio",
      title: "Tres decisiones sobre la misma imagen",
      description: "Cambiar dimensiones y calidad modifica el peso de forma distinta. Compara las salidas antes de elegir una receta para tu componente.",
      attribution: "Muestra propia de ZoePic; no contiene material de terceros.",
      methodology: "Se mantiene el mismo PNG y se comparan dos tamaños y dos valores de calidad.",
      limitations: "Las cifras sirven para comparar esta muestra, no para prometer un porcentaje para otros archivos.",
      variants: [
        { label: "Doble densidad · calidad 82", width: 1200, height: 800, quality: 82 },
        { label: "Tarjeta · calidad 82", width: 960, height: 640, quality: 82 },
        { label: "Doble densidad · calidad 65", width: 1200, height: 800, quality: 65 },
      ],
    },
    sections: [
      { heading: "Empieza por el espacio de destino", paragraphs: ["El tamaño correcto nace del componente, no de la cámara. Si una tarjeta nunca supera 640 píxeles de ancho, publicar un archivo de 4000 píxeles obliga al visitante a descargar datos que no verá. Mide el ancho máximo del componente y considera pantallas de alta densidad cuando la imagen lo requiera.", "Una regla de trabajo razonable consiste en exportar hasta el doble del ancho CSS para imágenes importantes en pantallas densas. No siempre hace falta: miniaturas, fondos con poco detalle y conexiones lentas pueden beneficiarse de variantes más pequeñas.", "En la prueba de esta página comparamos la misma muestra a 1200 × 800 con calidad 82, a 960 × 640 con calidad 82 y de nuevo a 1200 × 800 con calidad 65. Así se separa el efecto de reducir píxeles del efecto de cambiar la compresión; el recorte y el detalle visual deben revisarse en el tamaño real de uso."] },
...
export function getGuideReadingTime(guide: Guide): string {
  const wordCount = [guide.title, guide.description, ...guide.sections.flatMap((section) => [section.heading, ...section.paragraphs, ...(section.bullets ?? [])])]
    .join(" ")
    .trim()
    .split(/\s+/)
